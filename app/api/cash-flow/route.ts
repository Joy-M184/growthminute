...
export async function GET(request: Request) {
  ...
  const transactions = await getDb().select().from(cashTransactions)
    .where(and(eq(cashTransactions.ownerId, userId), eq(cashTransactions.mode, mode)))
...
export async function POST(request: Request) {
  try {
    ...
    const [transaction] = await getDb().insert(cashTransactions).values({ ownerId: userId, mode: payload.mode, type: payload.type, amountPence, category: category.slice(0, 80), note: payload.note?.trim().slice(0, 240) ?? "", transactionDate: transactionDate! }).returning();
...
export async function DELETE(request: Request) {
  ...
  await getDb().delete(cashTransactions).where(and(eq(cashTransactions.id, id), eq(cashTransactions.ownerId, userId)));
